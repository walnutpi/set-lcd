...
static void gt9xx_ts_irq_poll_timer(struct timer_list *t)
{
	struct goodix_ts_data *tsdata = from_timer(tsdata, t, timer_work);

	schedule_work(&tsdata->work_i2c_poll);
	mod_timer(&tsdata->timer_work, jiffies + msecs_to_jiffies(POLL_INTERVAL_MS));
}

static void gt9xx_ts_work_i2c_poll(struct work_struct *work)
{
...
static int gtp_request_irq(struct goodix_ts_data *ts)
{
	...
	else
	{
		...
		INIT_WORK(&ts->work_i2c_poll,
				  gt9xx_ts_work_i2c_poll);
		timer_setup(&ts->timer_work, gt9xx_ts_irq_poll_timer, 0);
		ts->timer_work.expires =
			jiffies + msecs_to_jiffies(FIRST_POLL_DELAY_MS);
		add_timer(&ts->timer_work);
		ret = 0;
	}
	return ret;
}
...
static int gtp_find_valid_cfg_data(struct goodix_ts_data *ts)
{
	...
	dev_dbg(&ts->client->dev,
			"Config Groups\' Lengths: %d, %d, %d, %d, %d, Failed create work thread%d",
